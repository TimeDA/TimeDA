import java.io.*;
 ...
 public class GCJ2013_C /*implements Runnable */{
 ...
 //------------------------------------------------------------------------------
 	...
 	private void solveTest() throws IOException {
 		...
 		long b = readLong();
 		int left = (int) sqrt(a);
 		while ((long)left*left >= a) left--;
 		while ((long)left*left < a) left++;
 		int right = (int) sqrt(b);
 		while ((long)right*right <= b) right++;
 		while ((long)right*right > b) right--;
