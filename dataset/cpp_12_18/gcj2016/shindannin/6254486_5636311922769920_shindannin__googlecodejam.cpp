// topcoder.cpp : R\[ AvP[ṼGg |Cg`܂B
 ...
 // {
 // 	sa->runNumLoops(min(10000000, pow(SZ(ss.mSubPnts), 2))); // ƂɂŒł悢B
 ...
 static RandXor  randxor;    // }`XbhΉɂȂA؂ƂɗpIuWFNgpӂāAV[hς܂傤B
 ...
 int main(){
 	...
 	for (int testcase = 0; testcase < T; ++testcase)
 	{
 		...
 		const ll		msec = 1000;
 		{
 			const ll saTimeStart = getTime();            // ĂȂ܂JnBgetTiméAԂԂ֐
 			const ll saTimeEnd = saTimeStart + msec;     // ĂȂ܂IBm_startTime͂̃vÔ̊Jn
 			ll saTimeCurrent = saTimeStart;          // ݂̎
 
 			int numLoops;
 			while ((saTimeCurrent = getTime()) < saTimeEnd) // ĂȂ܂I܂Ń[v
 			{
 				for (int saloop = 0; saloop < 100; ++saloop)
 				{
 					...
 					// ŏt=0̂Ƃ́AXRAǂȂ낤Ȃ낤Anextgp
 					// Ōt=T̂Ƃ́AXRAPƂ̂݁Anextgp
 					//					const ll duration = saTimeEnd - saTimeStart;         // ĂȂ܂ɂ鎞
 					//					const ll t = saTimeCurrent - saTimeStart;     // ĂȂ܂@Jn̎
 					//					const ll R = 10000;
 					// XRAȂƂłȀԂɈړJڂꍇtrueBP֐gpB
 					// TODO! expo[WI
 ...
 // 񍀊miBinomial Probabilityj
 const int COMB_MAX_N = 3005;
 const int COMB_MAX_K = 3005;
 static long double binoProb[COMB_MAX_N][COMB_MAX_K];	// n̎sŐmpk񐬌
 static long double binoMoreK[COMB_MAX_N][COMB_MAX_K];	// n̎sŐmpkȏ㐬
