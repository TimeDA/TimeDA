...
//        System.err.println("A="+A);
        ...
        boolean[][] has = new boolean[cnt.length][cnt[0].length];
        for (int i = 0; i < 1000; i++) {
            int x=0, y=0, v=9;
            for (int nx = 2; nx < R; nx++) {
                for (int ny = 2; ny < C; ny++) {
                    if (cnt[nx][ny] < v) {
                        v = cnt[nx][ny];
                        x = nx;
                        y = ny;
...
//            System.err.println("send "+x+","+y+"  "+v);
            out.printf("%d %d\n", x, y);
            String[] ss = LINE().split(" ");
            int rx = INT(ss[0]);
            int ry = INT(ss[1]);
            if (rx == 0 && ry == 0) {
//                System.err.println("done at "+i);
                return;
            }
            if (rx < 0) {
...
//            System.err.println("get  "+rx+","+ry);
            has[rx][ry] = true;
            for (int nx = rx-1; nx <= rx+1; nx++) {
                for (int ny = ry-1; ny <= ry+1; ny++) {
                    cnt[nx][ny]++;
                }
            }
...
//                System.err.printf("%2d: ", j);
//                System.err.println(Arrays.toString(cnt[j]));
//            }
//            System.err.println("### has");
//            for (int j = 0; j < has.length; j++) {
//                System.err.printf("%2d: ", j);
//                System.err.println(Arrays.toString(has[j]));
//            }
