#include <bits/stdc++.h>
 ...
 static int recurse(int &pool, vector<int> &ids, int cur, int parent, const vector<vector<edge>> &edges, vector<int> &out)
 {
     ids[cur] = pool++;
     int up = 0;
     for (const edge &e : edges[cur])
     {
         ...
         if (ids[e.trg] == -1)
         {
             int flow = recurse(pool, ids, e.trg, abs(e.lbl), edges, out);
             if (flow == 0)
                 throw bridge();
             out[abs(e.lbl)] = (e.lbl > 0) ? flow : -flow;
             up += flow;
         }
         else if (ids[e.trg] > ids[cur])
             up--;
         else
         {
             out[abs(e.lbl)] = (e.lbl > 0) ? 1 : -1;
             up++;
         }
     }
     return up;
 ...
 static void solve_case(int cas, ostream &cout, unique_ptr<promise<void> > &&input_done)
 {
     ...
     for (int i = 0; i < P; i++)
     {
         ...
         a--;
         b--;
         edges[a].push_back(edge{b, i + 1});
         edges[b].push_back(edge{a, -1 - i});
     }
     ...
     try
     {
         for (int i = 0; i < N; i++)
             if (ids[i] == -1)
             {
                 int up = recurse(pool, ids, i, -1, edges, out);
                 assert(up == 0);
             }
         for (int i = 1; i <= P; i++)
             cout << ' ' << out[i];
