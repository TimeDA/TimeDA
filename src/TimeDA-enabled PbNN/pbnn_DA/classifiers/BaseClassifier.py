...
class BaseClassifier:
    ...
    def _split_train_test(self, loader: PathMinerLoader, fold_ind: Union[int, Tuple[int, int]], pad: bool = False) \
        ...
        #train_labels = self._loader.train_labels()
        #test_labels = self._loader.test_labels()
    ...
    def cross_validation_folds(self) -> List[int]:
        ...
        if isinstance(test_size, float):
            print("int(np.ceil(1. / test_size))")
            return list(range(int(np.ceil(1. / test_size))))

        else:
            print("else")
            return list(range((int(np.ceil(max([inds.size for inds in self.__indices_per_class[self.__chosen_classes]]) / test_size)))))
