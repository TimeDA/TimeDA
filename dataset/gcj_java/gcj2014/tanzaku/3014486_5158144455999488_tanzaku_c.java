package r2;
 ...
 //		public static final String IN_FILE = "C-large.in";
 	...
 	class ParallelSolver implements Runnable {
 		...
 		int rec(int x, int y) {
 			if(x < 0 || x >= W) return 0;
 			if(ok[y][x] != -1) { return ok[y][x]; }
 			if(bu[y][x] || y + 1 < H && bu[y+1][x]) return ok[y][x] = 0;
 			if(y == 0) return ok[y][x] = 1;
 			for(int i = -1; i <= 1; i++) {
 				if(rec(x + i, y - 1) == 1) {
 					return ok[y][x] = 1;
 				}
 		...
 		public void run() {
 			...
 			ok = new int[H][W];
 			bu = new boolean[H][W];
 			for(int[] o : ok) Arrays.fill(o, -1);
 			for(int[] xs : b) {
 				for(int y = xs[1]; y <= xs[3]; y++) {
 			...
 			*/
 			...
 			for(int i = 0; i < B; i++) {
 				for(int j = i + 1; j < B; j++) {
 					...
 					if(b[i][2] + 1 <= b[j][0] || b[j][2] + 1 <= b[i][0]) {
 						v = Math.min(v, Math.min(Math.abs(b[i][0] - b[j][2] - 1), Math.abs(b[i][2] + 1 - b[j][0])));
 					}
 					if(b[i][3] + 1 <= b[j][1] || b[j][3] + 1 <= b[i][1]) {
 						v = Math.min(v, Math.min(Math.abs(b[i][1] - b[j][3] - 1), Math.abs(b[i][3] + 1 - b[j][1])));
 					}
 					dist[i][j] = dist[j][i] = v;
 					dp[i][j] = dp[j][i] = v;
 				}
 			...
 			for(int i = 0; i < B; i++) {
 				for(int j = 0; j < B; j++) {
 					ans = Math.min(ans, b[i][0] + dp[i][j] + W - 1 - b[j][2]);
 //					out.println(b[i][0] + " " + b[j][2] + " " + dp[i][j]);
 				}
 			}
 			out.println(ans);
 		}
