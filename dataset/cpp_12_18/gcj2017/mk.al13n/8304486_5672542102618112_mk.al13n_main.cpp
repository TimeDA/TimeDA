#define _CRT_SECURE_NO_DEPRECATE
 ...
 const int pw10[10]={1,10,100,1000,10000,100000,1000000,INF/100,INF/10,INF};
 ...
 void precalc(int n){
   ...
   forn(a,pw10[n]-1){
     if(valid(a+1,n))
       v.pb(mp(prec(a+1,n),a+1));
   }
   sort(all(v));
   pre[n].resize(pw10[n],1);
   for(int i=sz(v)-1;i>=0;--i){
     ...
     if(a==pw10[n-1]){
       ...
       //dbg(n,a,g,prec(a,n),prec(g,n));
       //assert(prec(g,n)<prec(a,n));
       pre[n][g]+=pre[n][a];
     }
   }
   dbg(n);
