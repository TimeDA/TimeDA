import static java.lang.Math.*;
 ...
 //    private static final String IMPOSSIBLE = "IMPOSSIBLE";
     ...
     private void run(int caseNumber) throws Exception {
         ...
         for (int[] pick : combinations(a, K)) {
             double[] p = new double[K/2+1];
             p[0] = 1;
             for (int i : pick) {
                 double yes = P[i];
 ...
 //        out.println();
     ...
     public static int[][] combinations(int[] a, int k) {
         ...
         comb(a,0,new int[k],0,ls);
         return ls.toArray(new int[ls.size()][]);
     }
     
     private static void comb(int[] a, int i, int[] pick, int pi, List<int[]> ls) {
         if (pi==pick.length) {
             ls.add(pick.clone());
             return;
         }
         ...
         // choose ai
         pick[pi]=a[i];
         comb(a, i+1, pick, pi+1, ls);
         // skip ai
         comb(a, i+1, pick, pi, ls); 
     }
