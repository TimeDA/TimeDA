...
//        if (all%(H+1) != 0 || all%(V+1) != 0 || all %((H+1)*(V+1)) != 0) {            
        if (all %((H+1)*(V+1)) != 0) {            
            out.printf("Case #%d: %s\n", caseNumber, NO);
            return;
        }
        ...
        int cntHor = all/(H+1);
        int[] cutBegRow = new int[H+2];
        int s = 0;
        for (int i = 0; i < rowCnt.length; i++) {
            s += rowCnt[i]; 
            if (s%cntHor==0) {
                cutBegRow[s/cntHor] = i+1;
            }
        }
//        System.out.println("  cutRow[" + cutBegRow.length + "]=" + Arrays.toString(cutBegRow));
        for (int i = 1; i < cutBegRow.length; i++) {
            if (cutBegRow[i] == 0) {
                out.printf("Case #%d: %s\n", caseNumber, NO);
                return;                
            }
        ...
        for (int c = 0; c < C; c++) {
            ...
            if (s%cntVer==0) {
                cutBegCol[s/cntVer] = c+1;
            }
        }
//        System.out.println("  cutBegCol[" + cutBegCol.length + "]=" + Arrays.toString(cutBegCol));
        for (int i = 1; i < cutBegCol.length; i++) {
            if (cutBegCol[i] == 0) {
                out.printf("Case #%d: %s\n", caseNumber, NO);
                return;                
            }
        ...
        int cntPiece = all/(H+1)/(V+1);
        for (int i = 0; i < cutBegRow.length-1; i++) {
            for (int j = 0; j < cutBegCol.length-1; j++) {
                int v = 0;
                for (int r = cutBegRow[i]; r < cutBegRow[i+1]; r++) {
                    for (int c = cutBegCol[j]; c < cutBegCol[j+1]; c++) {
                        if (has[r][c]) {
                            v++;
                        }
                    }
