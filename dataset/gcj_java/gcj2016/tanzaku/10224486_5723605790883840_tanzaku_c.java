package R2;
 ...
 public class C {
 	...
 	class ParallelSolver implements Runnable {
 		...
 		final int L = 3;
 		void slow() {
 			int[] sc = new int[pair.length];
 			int[] sd = new int[pair.length];
 			for(int i = 0; i < w; i++) {
 				sc[i] = i;
 				sd[i] = U;
 			}
 			...
 			for(int i = w + h; i < w + h + w; i++) {
 				sc[i] = sc[i-1] - (i==w+h?0:1);
 				sd[i] = D;
 			}
 			for(int i = w + h + w; i < w + h + w + h; i++) {
 				sc[i] = sc[i-1] - (i==w+h+w?0:w);
 				sd[i] = L;
 			}
 			out.println();
 //			System.err.println(Arrays.toString(sc) + " " + Arrays.toString(sd));
 			for(maze = 0; maze < 1 << (w*h); maze++) {
 //				if(maze != 2) continue;
 				boolean ok = true;
 				for(int j = 0; j < pair.length; j++) {
 //					System.err.println(maze + " " + j + " " + pair[j] + ": " + (sc[pair[j]]*4+sd[pair[j]]) + " " + dfs(sc[j], sd[j]));
 					if(sc[pair[j]]*4+sd[pair[j]] != dfs(sc[j], sd[j])) {
 						ok = false;
 						break;
 					}
 				}
 		...
 		int dfs(int c, int d) {
 			if((maze>>>c&1) == 0) {
 				// '/'
 				if(d == U) {
 					if(c % w == 0) return c*4+L;
 					return dfs(c - 1, R);
 				}
 				if(d == L) {
 					if(c / w == 0) return c*4+U;
 					return dfs(c - w, D);
 				}
 				if(d == D) {
 					if(c % w == w-1) return c*4+R;
 					return dfs(c + 1, L);
 				}
 				if(d == R) {
 					if(c / w == h-1) return c*4+D;
 					return dfs(c + w, U);
 				}
 			} else {
 				// '\'
