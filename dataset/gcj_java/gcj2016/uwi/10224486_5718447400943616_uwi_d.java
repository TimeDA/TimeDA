 ...
 //	static String INPATH = BASEPATH + TParallel.class.getSimpleName().charAt(0) + (LARGE ? "-large-practice.in" : "-small-practice.in");
 	...
 	public void process() // parallelized!
 	{
 		...
 		for(int i = 0;i < 1<<n*n;i++){
 			if((ptn&i) == 0 && Integer.bitCount(i) < ret){
 				if(simulate(ptn|i)){
 	...
 	boolean simulate(int ptn)
 	{
 		...
 		do{
 			if(!dfs(ptn, 0, a, (1<<n)-1))return false;
 		}while(nextPermutation(a));
 		return true;
 	}
 	
 	boolean dfs(int ptn, int cur, int[] ord, int rem)
 	{
 		if(cur == ord.length)return true;
 		int row = ptn>>>n*ord[cur];
 		if((row&rem) == 0)return false;
 		int w = row&rem;
 		for(int i = 0;i < n;i++){
 			if(w<<~i<0){
 				if(!dfs(ptn, cur+1, ord, rem^1<<i))return false;
 			}
 		}
 		return true;
 	}
