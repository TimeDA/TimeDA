#include <algorithm>
 ...
 void solveCase() {
   ...
   // proceed
   err=scanf("%d", &N);
   FOR(n,0,N-1) err=scanf("%d%d", &left[n], &right[n]);
   FOR(n,0,N) left[n]--, right[n]--;
 ...
 #define MAXIT 4000
   ...
   for(; iter<MAXIT && at != N-1; iter++) {
       //if(cnum == 0 && iter < 10) printf("iter = %d at = %d\n", iter, at);
       swap(left[at], right[at]);
       at = right[at];
 
   }
   
   printf(iter < MAXIT ? "Case #%d: %d\n" : "Case #%d: Infinity\n", cnum+1, iter);
   }
