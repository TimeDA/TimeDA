 ...
 public class C {
 	...
 	class ParallelSolver implements Runnable {
 		...
 		public void readInput(final IOFast io) throws IOException {
 			...
 			for(int i = 0; i < n; i++) {
 				words[i] = new String(io.nextLine()).split(" ");
 //				System.err.println(Arrays.toString(words[i]));
 				wi[i] = new int[words[i].length];
 				for(int j = 0; j < words[i].length; j++) {
 					if(!mp.containsKey(words[i][j])) {
 						mp.put(words[i][j], mp.size());
 					}
 					wi[i][j] = mp.get(words[i][j]);
 				}
 			}
 			size = mp.size();
 		}
 		...
 		public void run() {
 			...
 			for(int i = 0; i < 1<<n; i++) {
 				if((i & 3) != 2) continue;
 				gid++;
 //				HashSet<Integer> en = new HashSet<>();
 //				HashSet<Integer> fr = new HashSet<>();
 				for(int j = 0; j < n; j++) {
 //					HashSet<Integer> set = (i>>>j&1) == 0 ? en : fr;
 					int[] id = (i>>>j&1) == 0 ? ide : idf;
 					for(int s : wi[j]) id[s] = gid;
 				}
 				int dup = 0;
 //				for(int s : en) if(fr.contains(s)) dup++;
 				for(int j = 0; j < size; j++) if(ide[j] == gid && idf[j] == gid) dup++;
 				ans = Math.min(ans, dup);
 			}
 			out.println(ans);
 		}
