import static java.lang.Math.*;
 ...
 //            S[i] = INT();
         ...
         sort(a);
 //        double res = binSearch(D, a);
         double res = 1e18;
         for (int i = 0; i < a.length; i++) {
             double s = 1.0*D*a[i].s/(D-a[i].k);
             res = Math.min(res, s);
         }
 ...
 //        out.println();
     ...
     private boolean ok(double s, XY[] a, int D) {
         for (int i = 0; i < a.length; i++) {
             if (s <= a[i].s) {
                 continue;
             }
             
             double t = a[i].k/(s-a[i].s);
             if (t*s < D-EPS) {
                 return false;
             }
         }
     ...
     static class XY implements Comparable<XY> {
         ...
         public int compareTo(XY other) {
             ...
             return s < other.s ? -1 : (s == other.s ? 0 : 1);
         }
     }
