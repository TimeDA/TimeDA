#include <bits/stdc++.h>
 ...
 static void solve_case(int cas, ostream &cout, unique_ptr<promise<void> > &&input_done)
 {
     ...
     for (const auto &a : act)
         used[a.user] += a.stop - a.start;
     vector<int> gaps[2];
     for (int i = 0; i < SZ(act) - 1; i++)
     {
         if (act[i].user == act[i + 1].user)
             gaps[act[i].user].push_back(act[i + 1].start - act[i].stop);
         else
             swap++;
     }
     if (act[0].user == act.back().user)
         gaps[act[0].user].push_back(act[0].start + 1440 - act.back().stop);
     else
         swap++;
     for (int i = 0; i < 2; i++)
     {
         sort(RA(gaps[i]), greater<int>());
         while (!gaps[i].empty() && gaps[i].back() + used[i] <= 720)
         {
             used[i] += gaps[i].back();
             gaps[i].pop_back();
         }
