...
pair<int, int> StupidSolution() {
    ...
    pair<int, set<int>> ans(0, set<int>());
    for (const auto& xx : x) {
        for (const auto& yy : y) {
            int cur = 0;
            forn(i, n) {
                if (xx == d[i] + a[i] || yy == d[i] - b[i]) {
                    ++cur;
                    if (cur > ans.first) {
                        ans = mp(cur, set<int>());
                    }
    ...
    return mp(ans.first, int(ans.second.size()));
}
...
void Solve(bool skipThisTest) {
    ...
    const pair<int, int> ans = StupidSolution();
    cout << ans.first << " " << ans.second << endl;

    cerr << "\tclever answer is '" << ans << "'." << endl;
    if (n <= 300) {
        const pair<int, int> stupidAnswer = StupidSolution();
