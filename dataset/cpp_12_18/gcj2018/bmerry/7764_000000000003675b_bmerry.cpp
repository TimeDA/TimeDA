...
static void solve_case(int cas, ostream &cout, unique_ptr<promise<void> > &&input_done)
{
    ...
    for (int i = 0; i < N; i++)
        ...
        {
            int P = signs[i].first;
            int Q = signs[j].second;
            int run = 0;
            for (int tail = 0; tail <= N; tail++)
            {
                if (tail == N || (signs[tail].first != P && signs[tail].second != Q))
                {
                    if (run > ans)
                    {
                        ans = run;
