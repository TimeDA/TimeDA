#include <bits/stdc++.h>
 ...
 struct ticket
 {
     ...
     bool operator<(const ticket &other) const { return p < other.p; }
 };
 
 static int assign(const vector<ticket> &tickets, int N, int R)
 {
     vector<int> req(N);
 ...
 static void solve_case(int cas, ostream &cout, unique_ptr<promise<void> > &&input_done)
 {
     ...
     sort(RA(tickets));
     int lo = *max_element(RA(cticks)) - 1;
     int hi = M;
     while (hi - lo > 1)
     {
         int mid = (lo + hi) / 2;
         if (assign(tickets, N, mid) == -1)
             lo = mid;
         else
             hi = mid;
     }
     int cost = assign(tickets, N, hi);
     cout << "Case #" << cas + 1 << ": " << hi << ' ' << cost << "\n";
 }
