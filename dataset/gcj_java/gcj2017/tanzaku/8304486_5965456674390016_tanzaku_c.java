 ...
 public class C {
 	...
 	class ParallelSolver implements Runnable {
 		...
 		int dfs(int v, int t) {
 			...
 			for (int i = 0; i < 2; i++) {
 				final Tour to = tour[v][i];
 				if (!to.vis) {
 					to.vis = true;
 					ans = Math.min(ans, dfs(to.e, to.arrive(t)));
 					to.vis = false;
 				}
 			}
 			if (ans == Integer.MAX_VALUE && finished()) ans = t;
 			return ans;
 		}
