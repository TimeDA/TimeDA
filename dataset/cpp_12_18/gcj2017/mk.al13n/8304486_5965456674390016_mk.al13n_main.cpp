 ...
 struct sol{
   ...
   sol(){
     ...
     forn(i,n*2){
       ...
       D0[i]=d;
       --E[i];
       V[E[i]].pb(mp((d+L[i])%24,i));
       add+=(d+L[i])/24;
     }
   }
 
   LL check(vector<int> F,int f1){
     LL res=0;
     int v=0,e=V[0][f1].Y;
     vector<int> W(n*2);
     forn(qq,n*2){
       //int ee=2*v+((e==V[v][1].Y));
     }
     return res;
   }
   ...
   void solve(){
     ...
     forn(i,n){
       assert(sz(V[i])==2);
       C[i*2+0]=(V[i][0].X>L[i*2])+(V[i][1].X>L[i*2+1]);
       C[i*2+1]=(V[i][1].X>L[i*2])+(V[i][0].X>L[i*2+1]);
     }
     //dbg(V);
     //dbg(C);
     forn(f0,2){
       forn(f1,2){
         ...
         while(true){
           ...
           forn(v,n){
             ...
             if(Z[v]!=1)
               continue;
             mn=min(mn,mp(v?C[v*2+(F[v]^1)]-C[v*2+F[v]]:2,v));
           }
           if(mn.X==3){
             assert(all2);
             break;
         ...
         forn(v,n){
           if(v)
             ...
             r+=V[v][f1].X + (L[v*2+(F[0]^f1^1)]<V[v][f1^1].X)*24;
         }
         //dbg(f0,f1,add,r,F);
         ans=min(ans,r);
       }
