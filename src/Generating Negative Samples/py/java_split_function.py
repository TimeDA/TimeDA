...
bas_var = ["byte", "short", "int", "long", "float", "double", "char", "boolean"]
bas_var_lib = {'byte': 'bytewrap', 'short': 'shortwrap', 'int': 'intwrap', 'long': 'longwrap', 'float': 'floatwrap',
               'double': 'doublewrap', 'char': 'charwrap', 'boolean': 'booleanwrap'}
charge_vars_list = []
vars_charged = []
...
# you need to seek() to that line first
...
def charge_var(var_type, var_name, p):
    decls = get_decl(p)
    for decl in decls:
        if len(decl) < 2: continue
        if decl[1].text == var_name:
            if len(decl) == 3:
                value = "new " + var_type + "(" + "".join(decl[2][0].itertext()) + ");\n"
            else:
                value = "new " + var_type + "();\n"
            index = decl.getparent().getparent().index(decl.getparent())
            block_con = decl.getparent().getparent()
            node = Element("expr")
            node.text = var_type + " " + var_name + "=" + value
            print(node.text)
            block_con.insert(index + 1, node)
            print(etree.tostring(block_con[index + 1]))
            decl.getparent().remove(decl)
    names = get_vars_names(p)
    for name in names:
        if name.text == var_name:
            name.text = var_name + ".val"

    if var_type not in vars_charged:
        vars_charged.append(var_type)
        class_str = "class " + var_type + "{\npublic int val;\npublic " + var_type + "(){}\npublic " + var_type + "(int val){\nthis.val = val;\n}\n}"
        class_node = Element('class')
        class_node.text = class_str
        unit_node = get_unit(p)
        unit_node.insert(-1, class_node)


def check_continue_break(func, line_start, line_end):
	continue_breaks = func.xpath('.//src:continue', namespaces=ns) + func.xpath('.//src:break', namespaces=ns)
...
def check_index(func, line_start, line_end):
    indexes = func.xpath('.//src:index', namespaces=ns)
    for elem in indexes:
        elem_line_start = get_elem_line_start(elem)
        if line_start <= elem_line_start <= line_end:
            return True
...
def split_func(xml_file, c_file, func_name, line_start, line_end, new_file):
    ...
    for k, v in new_vars.items():
        if not is_first:
            ...
            is_first = False
        if v in bas_var:
            charge_vars_list.append([bas_var_lib[v], k])  ######################
            new_func += (bas_var_lib[v].encode('utf8') + b' ' + k.encode('utf8'))
        else:
