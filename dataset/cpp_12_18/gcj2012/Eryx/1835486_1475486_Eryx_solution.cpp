#include <algorithm>
 ...
 void solveCase() {
   ...
   err = scanf("%d", &N);
   FOR(i,0,N) err=scanf("%d", &L[i]);
   FOR(i,0,N) err=scanf("%d", &P[i]);
   
   FOR(a,0,N) ord[a] = a;
   
   int i = 0;
   for(i=1; i<N; i++) if(i > 0 && L[ord[i]] * P[ord[i-1]] < L[ord[i-1]] * P[ord[i]]) 
     swap(ord[i], ord[i-1]), i-=2;
   
   printf("Case #%d:", cnum+1);
   FOR(u,0,N) printf(" %d", ord[u]);
   printf("\n");
   }
