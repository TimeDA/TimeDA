...
int dfs(int a,int b){
  if(a<0||b<0||a>=sz(B)||b>=sz(B[0])||B[a][b]!='#') return 0;
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    forn(i,n*2-1){
      forn(j,m*2-1){
        mss.insert((A[i/2][j/2]=='W')+(A[i/2][(j+1)/2]=='W')*2+(A[(i+1)/2][j/2]=='W')*4+(A[(i+1)/2][(j+1)/2]=='W')*8);
      }
    }
    int ans=0;
    forn(i0,n){
      forn(j0,m){
        forn(ms,1<<4){
          if(!mss.count(ms)) continue;
          forn(i,n) forn(j,m) B[i][j]=(A[i][j]=='W')==((ms>>((i>=i0)*2+(j>=j0)))&1) ? '#' : '.';
          forn(i,n){
            forn(j,m){
              int t=dfs(i,j);
              if(t>ans){
