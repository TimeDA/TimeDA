...
def transform_tmp_id_names(src_author, dst_author, ignore_list=[], save_to='tmp.xml', keep_log=False):
	...
	#print(intersect)
	#print(dst_vars_info)
	diff = {k : dst_vars_cnt[k] for k in set(dst_vars_cnt) - intersect - set(src_templates) - set(dst_templates)} #不能换成原作者已经有的变量名
	dst_vars_info = {k : dst_vars_info[k] for k in set(dst_vars_info) - intersect - set(src_templates) - set(dst_templates)} #不能换成原作者已经有的变量名
