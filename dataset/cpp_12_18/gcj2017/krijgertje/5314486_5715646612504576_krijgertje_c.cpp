#include <algorithm>  
 ...
 bool go(int x,int y,int k,int cid) {
 	while(true) {
 		//printf("going %d: (%d,%d,%d)\n",cid,x,y,k);
 		x+=DX[k],y+=DY[k];
 		if(x<0||x>=h||y<0||y>=w||g[x][y]=='#') return true;
 		if(g[x][y]=='.') { opt[x][y].PB(cid); continue; }
 ...
 bool solve() {
 	...
 	REP(x,h) REP(y,w) if(id[x][y]!=-1) {
 		if(!go(x,y,0,2*id[x][y]+0)||!go(x,y,2,2*id[x][y]+0)) invalid.PB(2*id[x][y]+0);
 		if(!go(x,y,1,2*id[x][y]+1)||!go(x,y,3,2*id[x][y]+1)) invalid.PB(2*id[x][y]+1);
 	}
