...
struct graph{
  ...
  graph(int n,int m):G(n),mt(m,-1),W(n){}
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    forn(q,2){
      sort(all(I[q]));
      I[q].erase(unique(all(I[q])),I[q].end());
    }
    graph G(sz(I[0]),sz(I[1]));
    forn(i,n){
      forn(j,n){
        G.add(lower_bound(all(I[0]),mp(i,A[i][j]))-I[0].begin(),
              lower_bound(all(I[1]),mp(j,A[i][j]))-I[1].begin());
      }
    }
    int r=G.match();
    cout<<n*n-r<<'\n';
