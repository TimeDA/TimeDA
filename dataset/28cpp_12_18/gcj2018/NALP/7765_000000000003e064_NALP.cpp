...
bool Gen(int pos, const vector<set<char>>& letters, const set<string>& words, string& cur) {
    if (pos == k) {
        return !words.count(cur);
    }

    cur += "#";
    for (const auto& letter : letters[pos]) {
        cur.back() = letter;
        if (Gen(pos + 1, letters, words, cur)) {
            return true;
        }
    }
...
void Solve(bool skipThisTest) {
    ...
    vector<set<char>> letters(k);
    forn(i, n) {
        forn(j, k) {
            letters[j].insert(a[i][j]);
        }
    }

    string ans;
    if (Gen(0, letters, set<string>(a, a + n), ans)) {
        cout << ans << endl;
    } else {
        cout << "-" << endl;
    }
