#include <bits/stdc++.h>
 ...
 struct route
 {
     ...
     int next(int now) const
     {
         int tnow = now % 24;
         if (tnow > leave)
             tnow -= 24;
         return now + (leave - tnow);
     }
 };
 ...
 static int recurse(vector<camp> &camps, int here, int now, int rem)
 {
     if (rem == 0)
         return now;
     int ans = INT_MAX / 2;
     for (int i = 0; i < 2; i++)
     {
         if (camps[here].used & (1 << i))
             continue;
         camps[here].used |= 1 << i;
         const route &r = camps[here].routes[i];
         int depart = r.next(now);
         int arrive = depart + r.time;
         ans = min(ans, recurse(camps, r.trg, arrive, rem - 1));
         camps[here].used ^= 1 << i;
     }
     return ans;
 }
 ...
 static void solve_case(int cas, ostream &cout, unique_ptr<promise<void> > &&input_done)
 {
     ...
     for (int i = 0; i < N; i++)
     {
         for (int j = 0; j < 2; j++)
         {
             route &r = camps[i].routes[j];
             cin >> r.trg >> r.leave >> r.time;
             r.trg--;
         }
     }
