package qual;
 ...
 public class C {
 	...
 	class ParallelSolver implements Runnable {
 		...
 		public void run() {
 			...
 			/*
 			for(int x = 0, xv = 1; x <= L * 4; x++, xv = mul(xv, toNum(cs[x%L]))) {
 				for(int y = 0, yv = 1; y <= L * 4; y++, yv = mul(yv, toNum(cs[(x+y)%L]))) {
 					for(int z = 0, zv = 1; z <= L * 4; z++, zv = mul(zv, toNum(cs[(x+y+z)%L]))) {
 						if((x + y + z) % L != 0) { continue; }
 						if(x + y + z >= L * X) { continue; }
 						
 						int loop
 						for(int i = 0; i < (x + y + z) / L % 4; i++) {
 							;
 						}
 					}
 				}
 ...
 //								if(i+j+k == 0) {
 //									System.err.println(i + " " + j + " " + k + ": " + (loopLen + k) + " " + len3);
 //								}
 								
 								if(loopLen + k <= X && (loopLen + k) % 4 == X % 4) {
 //									if(i+j+k == 0) {
 //										System.err.println(i + " " + j + " " + k + ": " + suffix[L-len3] + " " + loop3);
 //									}
