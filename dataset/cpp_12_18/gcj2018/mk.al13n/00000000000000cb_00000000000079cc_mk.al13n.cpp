...
struct vec{
  ...
  vec operator/(ld b)const{return vec(x/b,y/b,z/b);}

  ld dot(const vec&v)const{ return x*v.x+y*v.y+z*v.z; }
  ld lensq()const{ return dot(*this); }
  ld len()const{return sqrt(lensq());}
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    auto doit=[&](ld ang){
      ...
      v1=vec(-s,c,0);
      v2=vec(s2/2*c,s2/2*s,s2/2);
      return v1.y+v2.y*2;
    };

    ld mn=0,mx=PI/2-atan(s2/2);
    forn(q,100){
      ld a=(mx+mn)*.5;
      if(doit(a)>A) mx=a;
      else mn=a;
    }
    vec v[3]={v1/2,v2/2,vec(v2.x,v2.y,-v2.z)/2};
    cout<<'\n';
    forn(q,3) cout<<v[q].x<<' '<<v[q].y<<' '<<v[q].z<<'\n';
    ld s=0;
    forn(q,3){
      ld d=v[q].len();
      assert(fabs(d-.5)<1e-6);
      assert(fabs(v[q].dot(v[(q+1)%3]))<1e-8);
      s+=fabs(v[q].y);
    }
    assert(fabs(s*2-A)<1e-6);
  }
