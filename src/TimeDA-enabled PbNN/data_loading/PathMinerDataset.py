...
class PathMinerDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self._mode == 'nn':
            ...
            starts = torch.LongTensor(self._contexts.starts[index].astype(float))
            paths = torch.LongTensor(self._contexts.paths[index].astype(float))
            ends = torch.LongTensor(self._contexts.ends[index].astype(float))
            if self._should_pad:
