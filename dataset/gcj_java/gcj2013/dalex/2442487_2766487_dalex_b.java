import java.io.*;
 ...
 //------------------------------------------------------------------------------
 	...
 	private void solveTest() throws IOException {
 		long n = 1L << readInt();
 		long p = readLong();
 		long a = definitelyWin(n, p);
 		long b = possiblyWin(n, p);
 		out.println(a + " " + b);
 	}
 
 	private long definitelyWin(long n, long p) {
 		long left = 0, right = n - 1, ans = -1;
 		while (left <= right) {
 			long t = (left + right) / 2;
 			if (definitelyWin(n, p, t)) {
 				ans = t;
 				left = t + 1;
 			} else {
 				right = t - 1;
 	...
 	private boolean definitelyWin(long n, long p, long t) {
 		long place = 1;
 		for (long teams = n; teams >= 1; teams >>= 1) {
 			long lower = teams - t - 1;
 			long upper = t;
 			if (upper == 0) {
 				continue;
 			}
 	...
 	private long possiblyWin(long n, long p) {
 		long left = 0, right = n - 1, ans = -1;
 		while (left <= right) {
 			long t = (left + right) / 2;
 			if (possiblyWin(n, p, t)) {
 				ans = t;
 				left = t + 1;
 			} else {
 				right = t - 1;
 	...
 	private boolean possiblyWin(long n, long p, long t) {
 		long place = 1;
 		for (long teams = n; teams >= 1; teams >>= 1) {
 			long lower = teams - t - 1;
 			long upper = t;
