 ...
 public class B {
 	...
 	class ParallelSolver implements Runnable {
 		...
 		public void run() {
 			...
 			Arrays.fill(ev, 1);
 			for (int i = 0; i < m; i++) {
 				final int v = random.nextInt(1)+1;
 				ev[i] = v;
 				deg[es[i][0]] += v;
 				deg[es[i][1]] -= v;
 			}
 			...
 			for (; ;) {
 				...
 				for (int i = 0; i < n; i++) {
 					if (deg[i] != 0) bad = true;
 					for (int j = 0; j < m && deg[i] != 0; j++) {
 						final int d = deg[i];
 						if (es[j][0] == i && es[j][1] > i) {
 							// -deg[i]
 							if (ev[j] != d) {
 								ev[j] -= d;
 								deg[es[j][1]] += d;
