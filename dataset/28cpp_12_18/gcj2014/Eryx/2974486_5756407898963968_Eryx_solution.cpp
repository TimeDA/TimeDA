#include <algorithm>
 ...
 void solveCase() {
   int X, Y, x, y, fr[4], sr[4], tmp;
   err = scanf("%d", &Y);
   FOR(y, 0, 4) if(y == Y-1) {
     FOR(x, 0, 4) 
       err = scanf("%d", &fr[x]);
     } else FOR(x, 0, 4) err = scanf("%d", &tmp);
   err = scanf("%d", &Y);
   FOR(y, 0, 4) if(y == Y-1) {
     FOR(x, 0, 4) 
       err = scanf("%d", &sr[x]);
     } else FOR(x, 0, 4) err = scanf("%d", &tmp);
   /* FOR(t,0,4) printf("%d ", fr[t]); printf("\n");
   FOR(t,0,4) printf("%d ", sr[t]); printf("\n"); */
   int guess = 0;
   FOR(a, 0, 4) FOR(b,0,4) if(fr[a] == sr[b]) guess = fr[a];
   if(guess == 0) {
     printf("Case #%d: Volunteer cheated!\n", cnum);
     return;
     }
   FOR(a, 0, 4) FOR(b,0,4) if(fr[a] == sr[b] && fr[a] != guess) {
