...
bool solve(){
  ...
  if(h>=n || v>=m) return false;
  if(!sz(A)) return true;
  if(sz(A)%((h+1)*(v+1))) return false;
  int k=sz(A)/(h+1)/(v+1);
  assert(k);
  sort(all(A));
  vector<int> B;
  vector<pii> C(v,mp(0,m));
  forn(q,h+1){
    if(q&&A[q*k*(v+1)-1].X==A[q*k*(v+1)].X) return false;
    B.clear();
    for(int i=q*k*(v+1);i<(q+1)*k*(v+1);++i) B.pb(A[i].Y);
    sort(all(B));
    forn(i,v){
      C[i].X=max(C[i].X,B[(i+1)*k-1]+1);
      C[i].Y=min(C[i].Y,B[(i+1)*k]);
    }
  }
  for(pii p:C) if(p.Y<p.X) return false;
  return true;
}
...
int main(int argc, char **argv){
  ...
  cin>>tc;
  forn(qq,tc){
    cout<<"Case #"<<qq+1<<": "<<(solve()?"POSSIBLE":"IMPOSSIBLE")<<'\n';
  }
